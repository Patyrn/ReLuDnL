...
from Relu_DNL.Utils import TransitionPoint, get_mini_batches
...
def find_the_best_transition_point_benchmarks_worker(transition_point_x, train_X, train_Y, train_weights, model,
    ...
    average_profit = np.median(get_optimization_objective(pred_Y=pred_Ys, Y=train_Y,
...
class relu_ppo(nn.Module):
    ...
    def fit_dnl(self, train_X, train_Y, train_weights, val_X, val_Y, val_weights, benchmark_size, print_test=False,
        ...
        if self.is_val:
            val_regret, val_obj = self.get_regret(val_X, val_Y, val_weights, pool=mypool)
        ...
        if print_test:
            test_regret, test_obj = self.get_regret(test_X, test_Y, test_weights, pool=mypool)
        else:
            test_regret = -1
            test_obj = -1
            #
    ...
    def get_regret(self, X, Y, weights, opt_params=None, pool=None):
        ...
        return regret, np.median(optimal_objective_values)
