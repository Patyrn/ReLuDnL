...
from Params import ICON_SCHEDULING_EASY
from PredictPlustOptimizeUtils import compute_C_k, compute_F_k
from Relu_DNL.Utils import read_file
...
def compute_sampled_alpha_icon_energy(pred_ys, benchmark_y, opt_params):
    """
    ...
    for i, pred_y in enumerate(pred_ys.T):
        ...
        tov, pov, predicted_opt_items = compute_icon_energy_single_benchmark(pred_y, Y=benchmark_y,
...
def compute_sampled_alpha_icon_energy(pred_ys, benchmark_y, opt_params):
    ...
    for i, pred_y in enumerate(pred_ys.T):
        ...
        tov, pov, predicted_opt_items = compute_icon_energy_single_benchmark(pred_y,
                                                                                 Y=benchmark_y,
                                                                                   opt_params=opt_params)
...
def compute_profit_icon_pred(pred_Y, Y, opt_params):
    ...
    index = range(len(Y))
    for i, benchmark_pred_Y, benchmark_Y in zip(index, pred_Y, Y):
        profit = compute_profit_pred_y(benchmark_pred_Y=benchmark_pred_Y, benchmark_Y=benchmark_Y,
                                       opt_params=opt_params)
        profits[i] = profit
...
def compute_profit_pred_y(benchmark_pred_Y, benchmark_Y, opt_params):
    profit_alpha_k, __, __ = compute_icon_energy_single_benchmark(benchmark_pred_Y, benchmark_Y,
                                                                  opt_params)
