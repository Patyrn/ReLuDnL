...
from Relu_DNL.Utils import get_file_path, read_file
...
def weighted_knapsack_template(dest_file_name='',
                               ...
                               number_of_rows_qptl=174, ylim=None, isUnit=False, is_run_time=False, is_normalize=True,
                               unit_tag_intopt=""):
    """
    1:DNL
    2:DNL_MAX
    ...
    if models[8]:
        for c in capacities:
            file_names = []
            for kfold in range(total_folds):
                file_name = "Tests/Knapsack/weighted/spartan/heap_greedy_c" + str(c) + 'k' + str(
                    kfold) + 'DIVIDE_AND_CONQUER_GREEDY-0-1.csv'
                file_names.append(file_name)
            files_relu_ppo_folders.append(file_names)
    ...
    relu_ppo_regrets = np.zeros((len(capacities), total_folds))
    ...
    for index, c in enumerate(capacities):
        for kfold in range(total_folds):
            ...
            file_relu_ppo = files_relu_ppo_folders[index]
            ...
            relu_ppo_runtime, relu_ppo_regret = find_regrets_relu_ppo(file_relu_ppo[kfold])
            ...
            if is_normalize:
                ...
                relu_ppo_regrets[index, kfold] = relu_ppo_regret / baseline_regret
                ...
                relu_ppo_regrets[index, kfold] = relu_ppo_regret
            ...
            if is_run_time:
                ...
                relu_ppo_regrets[index, kfold] = relu_ppo_runtime
    fig = plt.figure()
    # ax1 = fig.add_subplot(1, 2, 1)
    # ind = np.arange(0, 6)
    # ax1.set_xticks(ind)
    ...
    regrets = np.array([np.mean(dnc_regrets, axis=1), np.mean(dp_regrets, axis=1), np.mean(spotree_regrets, axis=1), np.mean(greedy_regrets, axis=1),  np.mean(relu_ppo_regrets, axis=1),
                        np.mean(baseline_regrets, axis=1),
                        np.mean(spo_regrets, axis=1), np.mean(qptl_regrets, axis=1), np.mean(intopt_regrets, axis=1),

                        np.mean(max_regrets, axis=1)])
    errors = np.array([np.std(dnc_regrets, axis=1) ,  np.std(dp_regrets, axis=1), np.std(spotree_regrets, axis=1), np.std(greedy_regrets, axis=1),  np.std(relu_ppo_regrets, axis=1),
                       np.std(baseline_regrets, axis=1),
                       np.std(spo_regrets, axis=1), np.std(qptl_regrets, axis=1), np.std(intopt_regrets, axis=1),

                       np.std(max_regrets, axis=1)])
    models.extend([1])
    max_regret = np.max(np.mean(greedy_regrets, axis=1))
    for index, model in enumerate(models):
        ind_plot = index
    ...
    # ax2.xaxis.set_major_locator(mticker.FixedLocator(xtickslocs))
    labels = ['DnL' , 'DP' , 'SPO-Forest', 'DnL-Greedy','Relu_PPO', 'Ridge Regression',  'SPO-Relax', 'QPTL', 'IntOpt']
    ax2.legend(labels)
    if is_save:
        plt.savefig('../figs/' + str(dest_file_name))
    if is_show:
...
def find_regrets_knapsack_dp(file_dp, is_run_time=False):
    ...
    min_val_test_regret_ddp, min_val_col_ddp, min_val_row_ddp = get_min_val_test_regret(val_regrets_dp, test_regrets_dp,
                                                                                        min_len)
    if is_run_time:
...
def find_regrets_relu_ppo(file_relu_ppo):
    run_times_relu, sub_epochs_relu, test_regrets_relu, val_regrets_relu = read_knapsack_files_relu(file_relu_ppo)

    min_len = min([len(arr) for arr in test_regrets_relu])
    min_val_test_regret_relu, min_val_col_relu, min_val_row_relu = get_min_val_test_regret(val_regrets_relu,
                                                                                           test_regrets_relu, min_len)

    min_val_run_time_relu = run_times_relu[min_val_row_relu][min_val_col_relu]
    return min_val_run_time_relu, min_val_test_regret_relu

def read_knapsack_files_relu(file_relu_ppo, header_length_pdo= 3):
    run_times = []
    sub_epochs = []
    test_regrets = []
    val_regrets = []

    df_list = read_file(filename=file_relu_ppo, folder_path="", delimiter=' ')
    ...
    for i in range(iter_no_pdo):
        ...
        df = df_list[start_index:end_index]
        run_times.append(np.array(df[2]).astype('float'))
        sub_epochs.append(np.max(np.array(df[0]).astype('float') + 1) * np.array(df[1]).astype('float') / np.max(np.array(df[1]).astype('float')))
        test_regrets.append(np.array(df[4]).astype('float'))
        val_regrets.append(np.array(df[5]).astype('float'))

    return run_times, sub_epochs, test_regrets, val_regrets

def find_regrets_knapsack(file_greedy, file_spo, file_qptl, facecolors, edgecolors,
                          models=[1, 1, 1, 1],
    ...
    run_times_spo, sub_epochs_spo, test_regrets_spo, val_regrets_spo = read_knapsack_files_spo(file_spo,
                                                                                               number_of_rows_spo)

    run_times_qptl, sub_epochs_qptl, test_regrets_qptl, val_regrets_qptl = read_knapsack_files_qptl(file_qptl,
                                                                                                    number_of_rows_qptl)
    ...
    # if models[0]:
    #     min_val_test_regret_dnc, min_val_col_dnc, min_val_row_dnc = get_min_val_test_regret(val_regrets_dnc,
    #                                                                                         test_regrets_dnc, min_len)
    if models[2]:
        min_val_test_regret_dnc_greedy, min_val_col_greedy, min_val_row_greedy = get_min_val_test_regret(
            val_regrets_greedy,
            test_regrets_greedy, min_len)
    if models[3]:
        min_val_test_regret_spo, min_val_col_spo, min_val_row_spo = get_min_val_test_regret(val_regrets_spo,
                                                                                            test_regrets_spo, min_len)
    if models[4]:
        min_val_test_regret_qptl, min_val_col_qptl, min_val_row_qptl = get_min_val_test_regret(val_regrets_qptl,
                                                                                               test_regrets_qptl,
                                                                                               min_len)
...
def mean_std_template(file_folder_predict, file_folder_relu, file_folder_spo, file_folder_qptl, files_predict, files_relu, files_spo, files_qptl,
    ...
    for index, file in enumerate(files_relu):
        df_list = read_file(filename=file, folder_path=file_folder_relu, delimiter=' ')
        start_indexes, end_indexes = find_indexes(df_list)
        number_of_rows_pdo = int((len(df_list) - 2 * header_length_pdo) / 2) - 1
        iter_no_pdo = int(len(start_indexes))
        for i in range(iter_no_pdo):
            ...
            end_index = start_index + 6
            df = df_list[start_index:end_index]
            run_times_relu.append(np.array(df[2]).astype("float"))
            sub_epochs_relu.append((np.max(np.array(df[0]).astype("float")) + 1) * np.array(df[1]).astype('float') / np.max(np.array(df[1]).astype('float')))
            test_regrets_relu.append(np.array(df[4]).astype("float"))
            val_regrets_relu.append(np.array(df[5]).astype("float"))
    ...
    if is_run_time:
        ...
        test_regrets_relu = run_times_relu
    ...
    y_axis_relu = test_regrets_relu


    val_regrets_relu = np.array(val_regrets_relu)
    test_regrets_relu = np.array(test_regrets_relu)

    tmp_val_relu_regrets = val_regrets_relu
    min_val_relu_row, min_val_relu_col = np.unravel_index(tmp_val_relu_regrets.argmin(), tmp_val_relu_regrets.shape)
    min_val_relu_test_regret = test_regrets_relu[min_val_relu_row, min_val_relu_col]
    # plt.scatter(x_axis[min_val_col], min_val_test_regret, color='b', ls='--')
    relu_regret = min_val_relu_test_regret
    ...
    # if is_show:
    #     plt.show()
    regrets = [baseline_regret, us_regret, relu_regret, spo_regret, qptl_regret]
    return regrets
