...
def one_hot_timeslot(len):
     ...
     return b.T
def get_benchmarks(X, Y, benchmark_size, generate_weight=True, unit_weight=True, weight_seed=None, add_weights=False,noise_level=0,  generate_optimization_feature= False):
    """
    ...
    benchmarks_Y = []
    benchmarks_weights = []
    benchmarks_opt_features =[]
    # generate opt features
    if generate_optimization_feature:

        opt_features = np.array([random.randint(0,1) for j in range(Y.shape[1])]).reshape(Y.shape)
        X = np.vstack((X, opt_features.flatten()))
        feature_size = feature_size + 1
        opt_feature_row = feature_size
    else:
        opt_features = None
    ...
    for i in range(benchmark_count):
        ...
        if generate_optimization_feature:
            benchmark_opt_features = opt_features[:, start_index:end_index].reshape(1, benchmark_size)
            benchmarks_opt_features.append(benchmark_opt_features)
    ...
    dataset = {'X': X,
               ...
               'benchmarks_opt_features': benchmarks_opt_features}
    return dataset
...
def get_benchmarks_spotree(X, Y, benchmark_size, generate_weight=True, unit_weight=True, weight_seed=None, add_weights=True,noise_level=0, generate_optimization_feature=False):
    """
    Splits the dataset into benchmarks of a certain size for the optimization problem. Used in the second stage.
    Might not be used in the feature if we choose to use predetermined benchmarks.
    :param is_weighted:
    ...
    #generate opt features
    if generate_optimization_feature:
        opt_features = np.array([x for x in range(Y.shape[1])])
        X = np.vstack((X, opt_features))
    else:
        optimization_features = None
