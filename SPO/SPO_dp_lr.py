...
from SPO.sgd_learner import DynamicRegression


class SGD_SPO_dp_lr:
    def __init__(self, capacity=None, weights=None, epochs=2, doScale= True, early_stopping= False,
        n_items=48,model=None, verbose=False, layer_params=None, plotting=False,return_regret= False,use_dp= True,use_relaxation=False,validation_relax= False,
        degree=1, optimizer= optim.Adam,store_result =False,**hyperparam):
    ...
    def fit(self, x_train, y_train,x_validation=None,y_validation=None,x_test=None,y_test=None):
        ...
        # network
        if not self.model:
            self.model = self.model = DynamicRegression(self.layer_params)  # input dim, output dim
            ...
            for kn_nr in knapsack_nrs:
                ...
                if self.verbose or self.plotting or self.store_result:
                    ...
                    if cnt % 20 == 0:
                        ...
                        if test:
                            if dict_validation['regret_full'] < min_val_regret:
                                #change this to revert
                                min_val_regret = dict_validation['regret_full']
                                ...
                                prev_dict_test = copy.deepcopy(dict_test)
                            else:
                                # print('hey', dict_validation['validation_regret_full'])
                                dict_test = prev_dict_test
