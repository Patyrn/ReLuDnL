...
def scheduling_script(load, layer_params):
    a = """#!/bin/bash
# Created by the University of Melbourne job script generator for SLURM
# Thu Aug 06 2020 16:14:44 GMT+1000 (Australian Eastern Standard Time)
...
export GRB_LICENSE_FILE=/usr/local/easybuild/software/Gurobi/gurobi.lic
time python /home/aguler/relu_dnl/SPO/test_scripts/icon_scheduling.py {} {}""".format(load, load, " ".join(map(str,layer_params)))

    file_name = file_folder + "icon_scheduling_l{}_param0{}.slurm".format(load, "0".join(map(str, layer_params)))
    with open(file_name, 'w') as f:
        f.write(a)

def scheduling_shell(loads, layer_params_list):
...
done""".format(loads_str, layer_params_str)
    with open("scheduling_script.sh", 'w') as f:
        f.write(a)

def knapsack_shell(layer_params_list):
    layer_params_str = ""
    for layer_params in layer_params_list:
        layer_params_str += " " + "0".join(map(str, layer_params))
    a="""#!/bin/sh
...
done""".format(layer_params_str)
    with open("knapsack_script.sh", 'w') as f:
        f.write(a)
def knapsack_script(layer_params):
    a = """#!/bin/bash
# Created by the University of Melbourne job script generator for SLURM
# Thu Aug 06 2020 16:14:44 GMT+1000 (Australian Eastern Standard Time)
...
export GRB_LICENSE_FILE=/usr/local/easybuild/software/Gurobi/gurobi.lic
time python /home/aguler/relu_dnl/SPO/test_scripts/icon_knapsack.py {}""".format(" ".join(map(str, layer_params)))

    file_name = file_folder + "icon_knapsack_param0{}.slurm".format("0".join(map(str, layer_params)))
    with open(file_name, 'w') as f:
        f.write(a)
...
if __name__ == '__main__':
    ...
    loads = [1,2,3]
    layer_params_list =[[5,1],[9,1],[5,5,1]]
    for load in loads:
            for layer_params in layer_params_list:
                scheduling_script(load, layer_params)
    scheduling_shell(loads,layer_params_list)
    ...
    for c in capacities:
            for layer_params in layer_params_list:
                knapsack_script(layer_params)

    knapsack_shell(layer_params_list)